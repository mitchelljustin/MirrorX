...
export function verifyStellarRefundTx({hash, xdr, holdingTx}) {
    const tx = new Stellar.Transaction(xdr)
    if (tx.hash().toString('hex') !== hash) {
        return null
    }
    const {timeBounds: {minTime}} = tx
    const refundTimestamp = parseInt(minTime)
    const holdingTimestamp = new Date(holdingTx.created_at).getTime() / 1000
    const refundDelta = holdingTimestamp - refundTimestamp
    // Grace period
    if (refundDelta >= 10 * 60) {
        return null
    }
    return tx
}
...
export async function processHoldingTx({
    ...
    return {refundTxHash, hashlock, tx}
}
...
export async function findStellarClaimTx({depositorAccount, holdingAccount, wait}) {
    const depositorExists = await stellarAccountExists(depositorAccount)
    if (!depositorExists) {
        throw new Error(`Depositor account does not exist. Please fund the depositor account with some XLM.`)
    }
    const {pagingToken} = await genPastLedgerPagingToken({delta: 1200})
    const filter = stellar
        .operations()
