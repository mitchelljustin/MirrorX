...
async function genFundHoldingTx({
                                    hashlock,
                                    swapSize,
                                    holdingAccount,
                                    depositorAccount,
    ...
    const ethAddrBuf = Buffer.from(withdrawerEthAddr.slice(2), 'hex')
    const ethAddrMemo = Buffer.alloc(32, 0x0)
    ethAddrBuf.copy(ethAddrMemo)

    const withdrawer = await stellar.loadAccount(withdrawerAccount)

    const memo = Stellar.Memo.hash(ethAddrMemo)
...
async function findHoldingTx({swapSize, holdingAccount, depositorAccount, wait}) {
    return new Promise(async (resolve, reject) => {
        ...
        const callBuilder = stellar.transactions()
            .forAccount(holdingAccount)
            .cursor(paging_token)
        if (wait) {
            const close = callBuilder.stream({
                onmessage: async (tx) => {
                    let info = await processLockFundsTx({tx, swapSize, holdingAccount, depositorAccount})
                    if (info === null) {
                        reject(`Not a valid holding transaction: ${tx.id}`)
                    } else {
                        resolve(info)
                    }
                    close()
            ...
            try {
                const tx = (await callBuilder.call()).records[0]
                let info = await processLockFundsTx({tx, swapSize, holdingAccount, depositorAccount})
                if (info === null) {
                    reject(`Not a valid holding transaction: ${tx.id}`)
                } else {
...
async function processLockFundsTx({tx, swapSize, holdingAccount, depositorAccount}) {
    ...
    let createSwapAcct, trustAsset, payAsset, addFulfiller, addHashlock, adjustSignerWeights
    const checks = [
        () => {
    ...
    const hasFunds = holding.balances.some(bal => {
        return bal.asset_code === XETH.getCode() &&
            bal.asset_issuer === XETH.getIssuer() &&
            BigNumber(bal.balance).eq(swapSize)
    })
    if (!hasFunds) {
        log(`Holding account does not have funds`)
        return null
    }
    const hashlockSigners = holding.signers.filter(
        signer => signer.type === 'sha256_hash'
    )
    if (hashlockSigners.length !== 1) {
        log(`Holding account does not have hashlock`)
        return null
    }
...
async function waitForEtherCommitment({swapSize, hashlock, withdrawerEthAddr}) {
    const targetHashlock = `0x${hashlock.toString('hex')}`
    const contract = await XMSwap.deployed()
    let done = false
    return new Promise((resolve, reject) => {
        contract.SwapPrepared().watch(scanForSwap)
        ...
        function scanForSwap(err, eventLog) {
            ...
            const validRecipient = eventLog.args.recipient.toLowerCase() === withdrawerEthAddr.toLowerCase()
            // TODO: check expiry
...
async function claimEther({withdrawerEther, preimage}) {
    const contract = await XMSwap.deployed()
...
async function awaitHoldingTx({holdingAccount}) {
...
class Withdraw {
    ...
    async genStellarSwapRefundTx({swapSize, swapKeys}) {
        ...
        const minTime = timestamp + WITHDRAW_REFUND_DELAY;
...
class Deposit {
    ...
    async processLockFundsTx({tx, swapSize}) {
        ...
        const checks = [
            ...
            () => trustAsset.asset_issuer === XETH.getIssuer(),
            () => payAsset.type === 'payment',
            () => BigNumber(payAsset.swapSize).eq(swapSize),
            () => payAsset.asset_code === XETH.getCode(),
            () => payAsset.asset_issuer === XETH.getIssuer(),
