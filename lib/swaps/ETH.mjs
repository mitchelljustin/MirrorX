...
import XMSwap from '../ethSwapContract'
...
async function findHoldingTx({swapSize, withdrawerAccount, holdingAccount, depositorAccount, wait}) {
    return new Promise(async (resolve, reject) => {
        const latestLedger = await stellar.ledgers().limit(1).order('desc').call()
        const {sequence} = latestLedger.records[0]
        const startSequence = sequence - 1200
        const startLedger = await stellar.ledgers().ledger(startSequence).call()
        const {paging_token} = startLedger
        const getWithdrawerTxs = stellar.transactions()
            .forAccount(withdrawerAccount)
            .cursor(paging_token)
        if (wait) {
            const close = getWithdrawerTxs.stream({
                onmessage: async (tx) => {
                    const info = await processLockFundsTx({tx, swapSize, holdingAccount, depositorAccount})
                    if (info === null) {
                        return
                    }
                    resolve(info)
            ...
            try {
                const txs = (await getWithdrawerTxs.call()).records
                for (const tx of txs) {
                    const info = await processLockFundsTx({tx, swapSize, holdingAccount, depositorAccount})
                    if (info === null) {
                        continue
