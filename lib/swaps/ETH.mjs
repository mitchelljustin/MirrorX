...
async function findEtherCommitment({swapSize, hashlock, withdrawerEthAddress, wait}) {
    hashlock = `0x${hashlock.toString('hex')}`
    swapSize = BigNumber(swapSize).times(1e18)
    const recipient = withdrawerEthAddress

    const contract = await EthSwapContract.deployed()
    const filter = contract.SwapPrepared(
        {
            hashlock,
            swapSize,
            recipient
...
function commitEthereumParams({hashlock, swapSize, depositorEthAddress, withdrawerEthAddress}) {
